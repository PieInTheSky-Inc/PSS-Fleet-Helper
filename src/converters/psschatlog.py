...
class PssChatLoggerConverter:
    ...
    async def to_text(
        self, for_admin: bool = False, bot: _model.PssApiDiscordBot = None
    ) -> _List[str]:
        ...
        if for_admin:
            ...
            if guild:
                result.append(
                    f"Guild - {guild.name} (ID: {self.pss_chat_log.guild_id})"
                )
                channel: _TextChannel = guild.get_channel(self.pss_chat_log.channel_id)
                if channel:
                    result.append(
                        f"Channel - {channel.name} (ID: {self.pss_chat_log.channel_id})"
                    )
                else:
                    result.append(
                        f"Channel - Error: cannot access channel (ID: {self.pss_chat_log.channel_id})"
                    )
            else:
                result.append(
                    f"Guild - Error: cannot access guild (ID: {self.pss_chat_log.guild_id})"
                )
        else:
            result.append(
                f"Channel - <#{self.pss_chat_log.channel_id}> (ID: {self.pss_chat_log.channel_id})"
            )
