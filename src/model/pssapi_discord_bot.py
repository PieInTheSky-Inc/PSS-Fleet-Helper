...
class PssApiDiscordBot(_commands.Bot):
    def __init__(
        self,
        *args,
        device_type: _pssapi.enums.DeviceType = None,
        language_key: _pssapi.enums.LanguageKey = None,
        ...
        **kwargs
        ...
        super().__init__(
            ...
            activity=_discord.activity.Activity(
                type=_discord.ActivityType.playing, name="fh help"
            ),
            *args,
            **kwargs
        )
    ...
    async def pssapi_login(self) -> _Optional[str]:
        ...
        checksum = self.pssapi_client.user_service.utils.create_device_login_checksum(
            device_id,
            self.pssapi_client.device_type,
            utc_now,
            _settings.PSS_DEVICE_LOGIN_CHECKSUM_KEY,
        )
        user_login = await self.pssapi_client.user_service.device_login_11(
            checksum,
            utc_now,
            device_id,
            self.pssapi_client.device_type,
            self.pssapi_client.language_key,
        )
