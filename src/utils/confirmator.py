...
class Confirmator:
    ...
    async def wait_for_option_selection(self) -> bool:
        def emoji_selection_check(reaction: _Reaction, user: _User) -> bool:
            if user != self.__context.bot.user:
                ...
                if (
                    emoji in Confirmator.reactions.keys()
                    and self.__reply.id == reaction.message.id
                ):
                    return True
            return False
        ...
        try:
            reaction, _ = await self.__context.bot.wait_for(
                "reaction_add",
                timeout=_DEFAULT_INQUIRE_TIMEOUT,
                check=emoji_selection_check,
            )
    ...
    async def __post_confirmation_message(self) -> _Message:
        self.__reply = await self.__context.reply(
            f"{self.__confirmation_message}\n\nDo you want to proceed?",
