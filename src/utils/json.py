...
class ViviDecoder(_json.JSONDecoder):
    def __init__(
        self,
    ):
...
class ViviEncoder(_json.JSONEncoder):
    def default(self, obj):
        if isinstance(obj, _datetime):
            return {
                "__type__": "datetime",
                "__value__": _format.datetime(
                    obj, include_time=False, include_tz=False, include_tz_brackets=False
                ),
            }
...
def yadc_decoder_object_hook(obj):
    if "__type__" in obj and "__value__" in obj:
        if obj["__type__"] in ["date", "datetime"]:
            return _parse.formatted_datetime(
                obj["__value__"],
                include_time=False,
                include_tz=False,
