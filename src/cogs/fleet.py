...
class Fleets(_CogBase):
    ...
    @_commands.group(
        name="fleet", aliases=["f"], brief="Set up fleets", invoke_without_command=True
    )
    ...
    async def add(self, ctx: _commands.Context, *, fleet_name: str) -> None:
        ...
        alliances = await self.bot.pssapi_client.alliance_service.search_alliances(
            access_token, fleet_name, 0, 100
        )
        ...
        alliances = [
            alliance for alliance in alliances if alliance.id not in existing_fleets_ids
        ]

        if not alliances:
            raise Exception(
                f"A fleet with the name `{fleet_name}` could not be found or has been already added."
            )
        ...
        if len(alliances) == 1:
            ...
            selector = _utils.Selector(
                ...
                fleet_name,
                alliances,
                _model.Fleet.get_alliance_search_description,
                "Select a fleet to add",
            )
        ...
        confirmator = _utils.Confirmator(
            ctx,
            f"Do you want to add the fleet {_model.Fleet.get_alliance_search_description(alliance)}?",
        )
        confirm = await confirmator.wait_for_option_selection()
        if confirm:
            short_name = None
            short_name, aborted, skipped = await _utils.discord.inquire_for_text(
                ctx,
                "Specify a short name for the fleet.",
                skip_text=f"No short name has been specified for the fleet {alliance.alliance_name}",
            )
            with _model.orm.create_session() as session:
            ...
            if short_name:
                await _utils.discord.send(
                    ctx,
                    f"The fleet **{alliance.alliance_name}** [{short_name}] (ID: {alliance.id}) has been added.",
                )
            else:
                await _utils.discord.send(
                    ctx,
                    f"The fleet **{alliance.alliance_name}** (ID: {alliance.id}) has been added.",
                )
        else:
    ...
    async def list(self, ctx: _commands.Context, *, fleet_name: str = None) -> None:
        ...
        if fleet_name:
            ...
            existing_fleets = [
                ...
                if fleet_name_lower in fleet.fleet_name.lower()
            ]
            if not existing_fleets:
                raise Exception(
                    "No fleet configured for this server matches the given fleet name."
                )

        existing_fleets = sorted(
            existing_fleets, key=lambda fleet: fleet.fleet_name or ""
        )
        ...
        for fleet in existing_fleets:
            ...
            if fleet.short_name:
                lines.append(
                    f"**{fleet.fleet_name}** [{fleet.short_name}] (ID: {fleet.id}), added: <t:{unix_timestamp}:D> <t:{unix_timestamp}:T>"
                )
            else:
                lines.append(
                    f"**{fleet.fleet_name}** (ID: {fleet.id}), added: <t:{unix_timestamp}:D> <t:{unix_timestamp}:T>"
                )
        await _utils.discord.send_lines(ctx, lines)

    @_commands.guild_only()
    @base.command(
        name="remove", aliases=["delete", "rem", "del"], brief="Remove a fleet"
    )
    async def remove(self, ctx: _commands.Context, *, fleet_name: str = None) -> None:
        ...
        if len(existing_fleets) == 1:
            ...
            selector = _utils.Selector(
                ...
                fleet_name,
                existing_fleets,
                _model.Fleet.get_fleet_search_description,
                "Select a fleet to remove",
            )
        ...
        confirmator = _utils.Confirmator(
            ctx,
            f"Do you want to remove the fleet {_model.Fleet.get_fleet_search_description(fleet)}?",
        )
        confirm = await confirmator.wait_for_option_selection()
        if confirm:
            with _model.orm.create_session() as session:
            ...
            await _utils.discord.send(
                ctx,
                f"The fleet **{alliance_name}** (ID: {alliance_id}) has been deleted.",
            )
        else:
            raise Exception("Process aborted by user.")
    ...
    async def update(
        ...
        if updated_fleets:
            ...
            await _utils.discord.send(
                ctx, f"Updated {len(updated_fleets)} configured fleets."
            )
        else:
