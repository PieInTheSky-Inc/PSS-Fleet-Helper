...
class ChatLogger(_CogBase):
    ...
    async def watch_log_chat(self):
        ...
        if not self.log_chat.is_running():
            ...
            if self.__last_log_chat_run_at is None:
                ...
                utc_now - self.__last_log_chat_run_at
            ).total_seconds > ChatLogger.__CHAT_LOG_INTERVAL:
                try:
                    self.log_chat.cancel()
                    print(
                        "[watch_log_chat] Successfully cancelled the task 'log_chat'."
                    )
                ...
                if not self.log_chat.is_running():
                    try:
                        self.log_chat.start()
                        print(
                            "[watch_log_chat] Successfully started the task 'log_chat'."
                        )
                    except Exception as ex:
    ...
    async def log_chat(self):
        ...
        try:
            with _model.orm.create_session() as session:
                pss_chat_loggers = _model.orm.get_all(
                    _model.chat_log.PssChatLogger, session
                )
            if not pss_chat_loggers:
                return
        except _psycopg2.OperationalError as ex:
            print(
                "[log_chat] Could not retrieve configured Chat Loggers from database:"
            )
            print(ex)
            return
        ...
        for pss_chat_logger in pss_chat_loggers:
            channel_keys.setdefault(pss_chat_logger.pss_channel_key, []).append(
                pss_chat_logger
            )
        channel_key_count = len(channel_keys.keys())

        remaining_time = (
            ChatLogger.__CHAT_LOG_INTERVAL
            - (_utils.datetime.get_utc_now() - utc_now).total_seconds()
        )
        delay = remaining_time / channel_key_count * (1 - channel_key_count * 0.01)

        for channel_key, pss_chat_loggers in channel_keys.items():
            tries = 2
            while tries > 0:
                try:
                    messages = await self.bot.pssapi_client.message_service.list_messages_for_channel_key(
                        access_token, channel_key
                    )
                    break
                    ...
                    print(
                        f"Could not get messages for channel key '{channel_key}':\n{ex}"
                    )
                    messages = None
                    tries -= 1
            ...
            if messages:
                ...
                for pss_chat_logger in pss_chat_loggers:
                    channel: _discord.TextChannel = self.bot.get_channel(
                        pss_chat_logger.channel_id
                    )
                    if channel:
                        messages = [
                            message
                            for message in messages
                            if message.message_id > pss_chat_logger.last_pss_message_id
                        ]
                        lines = []
                        for message in messages:
                            ...
                            lines.append(
                                f"{user_name_and_fleet}:** {_utils.discord.escape_markdown_and_mentions(message.message)}"
                            )
                        if lines:
                            try:
                                await _utils.discord.send_lines_to_channel(
                                    channel, lines
                                )
                            except:
                                continue
                            pss_chat_logger.last_pss_message_id = max(
    ...
    @_commands.group(
        name="chatlog", brief="Configure Chat Logging", invoke_without_command=True
    )
    ...
    async def add(
        ...
        *,
        ...
        log_channel = _model.chat_log.PssChatLogger.make(
            ctx.guild.id, channel.id, channel_key, name
        )
        with _model.orm.create_session() as session:
        ...
        await _utils.discord.reply(
            ctx, f"Posting messages from channel '{channel_key}' to {channel.mention}."
        )
    ...
    async def edit(self, ctx: _commands.Context, logger_id: int) -> None:
        ...
        if not pss_chat_logger:
            raise Exception(
                f"A chat log with the ID {logger_id} does not exist on this server."
            )
        ...
        new_channel_key, aborted, skipped_new_channel_key = (
            await _utils.discord.inquire_for_text(
                ctx, prompt_text, abort_text="Aborted", skip_text="Skipped."
            )
        )
        ...
        new_channel, aborted, skipped_new_channel = (
        ...
        new_name, aborted, skipped_new_name = await _utils.discord.inquire_for_text(
            ctx, prompt_text, abort_text="Aborted", skip_text="Skipped."
        )
        ...
        if edited:
            ...
            await _utils.discord.reply(
                ctx, f"The chat logger {pss_chat_logger} has not been edited."
            )

    @_commands.guild_only()
    @base.group(
        name="list",
        brief="List chat loggers for this server",
    ...
    async def list(self, ctx: _commands.Context) -> None:
        ...
        if ctx.invoked_subcommand is None:
            ...
            with _model.orm.create_session() as session:
                pss_chat_loggers = _model.orm.get_all_filtered_by(
                    _model.chat_log.PssChatLogger, session, guild_id=ctx.guild.id
                )
    ...
    async def delete(self, ctx: _commands.Context, logger_id: int) -> None:
        ...
        with _model.orm.create_session() as session:
            pss_chat_logger: _model.chat_log.PssChatLogger = (
                _model.orm.get_first_filtered_by(
                    _model.chat_log.PssChatLogger,
                    session,
                    id=logger_id,
                    guild_id=ctx.guild.id,
        ...
        delete_log, aborted, _ = await _utils.discord.inquire_for_true_false(
            ctx, prompt_text
        )
        ...
        if aborted:
            ...
            with _model.orm.create_session() as session:
                pss_chat_logger = _model.orm.get_by_id(
                    _model.chat_log.PssChatLogger, session, logger_id
                )
