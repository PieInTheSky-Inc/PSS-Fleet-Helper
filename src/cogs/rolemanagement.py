...
class RoleManagement(_CogBase):
    ...
    async def add(
        self, ctx: _commands.Context, role_to_add: _discord.Role, *, user_ids: str
    ) -> None:
        """
        ...
        if user_ids:
            ...
            confirmator = _utils.Confirmator(
                ctx,
                f"This command will add the role `{role_to_add}` to {len(user_ids)} members.",
            )
            ...
            if await confirmator.wait_for_option_selection():
                ...
                users_not_added = []
                reply = (
                    await _utils.discord.reply_lines(
                        ctx, [f"Adding role. Progress: 0/{len(user_ids)} members"]
                    )
                )[0]
                ...
                for i, user_id in enumerate(user_ids):
                    ...
                    if i and self.__print_progress(i):
                        _utils.discord.edit_lines(
                            reply,
                            [f"Adding role. Progress: {i}/{len(user_ids)} members"],
                        )
                ...
                if users_not_added:
                    lines.append(
                        f"Could not add role to {len(users_not_added)} users with ID:"
                    )
                ...
                if not _utils.discord.fits_single_message(lines):
                    ...
                    if users_not_added:
                        lines.append(
                            f"Could not add role to {len(users_not_added)} members."
                        )
    ...
    @role.group(
        name="addtorole",
        brief="Add Role to members with certain role(s)",
        invoke_without_command=True,
    )
    async def addtorole(
        self,
        ctx: _commands.Context,
        role_to_add: _discord.Role,
        required_role: _discord.Role,
        *,
        ...
        if required_roles:
            roles.extend(
                ctx.guild.get_role(role_id_or_mention)
        ...
        if members:
            ...
            confirmator = _utils.Confirmator(
                ctx,
                f"This command will add the role `{role_to_add}` to {len(members)} members.",
            )
            ...
            if await confirmator.wait_for_option_selection():
                ...
                reply = (
                    await _utils.discord.reply_lines(
                        ctx, [f"Adding role. Progress: 0/{len(members)} members"]
                    )
                )[0]
                ...
                for i, member in enumerate(members):
                    ...
                    if i and self.__print_progress(i):
                        await _utils.discord.edit_lines(
                            reply,
                            [f"Adding role. Progress: {i}/{len(members)} members"],
                        )
                ...
                if members_added_count < len(members):
                    lines.append(
                        f"Could not add role {role_to_add} to {len(members) - members_added_count} members."
                    )
    ...
    @role.group(
        name="clear",
        brief="Remove a role from all members",
        invoke_without_command=True,
    )
    async def clear(
        self, ctx: _commands.Context, role_to_remove: _discord.Role
    ) -> None:
        """
        ...
        if ctx.invoked_subcommand is None:
            ...
            if members:
                ...
                confirmator = _utils.Confirmator(
                    ctx,
                    f"This command will remove the role `{role_to_remove}` from {len(members)} members.",
                )
                ...
                if await confirmator.wait_for_option_selection():
                    ...
                    reply = (
                        await _utils.discord.reply_lines(
                            ctx, [f"Clearing role. Progress: 0/{len(members)} members"]
                        )
                    )[0]
                    ...
                    for i, member in enumerate(members):
                        ...
                        if i and self.__print_progress(i):
                            await _utils.discord.edit_lines(
                                reply,
                                [
                                    f"Clearing role. Progress: {i}/{len(members)} members"
                                ],
                            )
                    ...
                    if users_not_cleared:
                        lines.append(
                            f"Could not clear role {role_to_remove} from {len(users_cleared)} members:"
                        )
                    ...
                    if not _utils.discord.fits_single_message(lines):
                        ...
                        if len(users_cleared) < len(members):
                            lines.append(
                                f"Could not clear role {role_to_remove} from {len(members) - len(users_cleared)} members."
                            )

                    await _utils.discord.edit_lines(reply, lines)
            else:
                await _utils.discord.reply(
                    ctx, f"There are no members with the role {role_to_remove}."
                )
    ...
    async def clear_users(self, ctx: _commands.Context, *, user_ids: str) -> None:
        ...
        if user_ids:
            ...
            confirmator = _utils.Confirmator(
                ctx,
                f"This command will remove all non-managed roles from {len(user_ids)} members.\nNote: Roles that are above my highest role will not be removed.",
            )
            ...
            if await confirmator.wait_for_option_selection():
                ...
                reply = (
                    await _utils.discord.reply_lines(
                        ctx, [f"Clearing roles. Progress: 0/{len(user_ids)} members"]
                    )
                )[0]
                ...
                for i, user_id in enumerate(user_ids):
                    ...
                    if i and self.__print_progress(i):
                        await _utils.discord.edit_lines(
                            reply,
                            [f"Clearing roles. Progress: {i}/{len(user_ids)} members"],
                        )
    ...
    async def remove(
        self, ctx: _commands.Context, role_to_remove: _discord.Role, *, user_ids: str
    ) -> None:
        """
        ...
        if user_ids:
            ...
            confirmator = _utils.Confirmator(
                ctx,
                f"This command removes the role `{role_to_remove}` from {len(user_ids)} members.",
            )
            ...
            if await confirmator.wait_for_option_selection():
                ...
                reply = (
                    await _utils.discord.reply_lines(
                        ctx, [f"Removing role. Progress: 0/{len(user_ids)} members"]
                    )
                )[0]
                ...
                for i, user_id in enumerate(user_ids):
                    ...
                    if i and self.__print_progress(i):
                        await _utils.discord.edit_lines(
                            reply,
                            [f"Removing role. Progress: {i}/{len(user_ids)} members"],
                        )
                ...
                if not _utils.discord.fits_single_message(lines):
                    ...
                    if users_not_removed:
                        lines.append(
                            f"Could not remove role from {len(users_not_removed)} users."
                        )
    ...
    @role.group(
        name="removefromrole",
        brief="Remove a Role from members with certain role(s)",
        invoke_without_command=True,
    )
    async def removefromrole(
        self,
        ctx: _commands.Context,
        role_to_remove: _discord.Role,
        required_role: _discord.Role,
        *,
        required_roles: str = None,
    ) -> None:
        ...
        if members:
            ...
            if await confirmator.wait_for_option_selection():
                ...
                reply = (
                    await _utils.discord.reply_lines(
                        ctx, [f"Removing role. Progress: 0/{len(members)} members"]
                    )
                )[0]
                ...
                for i, member in enumerate(members):
                    ...
                    if i and self.__print_progress(i):
                        await _utils.discord.edit_lines(
                            reply,
                            [f"Removing role. Progress: {i}/{len(members)} members"],
                        )
                ...
                if users_removed_count < len(members):
                    lines.append(
                        f"Could not remove role {role_to_remove} from {len(members) - users_removed_count} members."
                    )
            ...
            await _utils.discord.reply(
                ctx,
                f"There are no members with the role to be removed matching the criteria.",
            )

    def __print_progress(self, current_count: int) -> bool:
        return not (current_count % RoleManagement.PRINT_PROGRESS_EVERY_X_MEMBERS)

    def __get_members_with_roles(
        self, ctx: _commands.Context, *roles: _List[_Union[int, _discord.Role]]
    ) -> _List[_discord.Member]:
